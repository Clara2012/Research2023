...
parser.add_argument('-p', '--path', default="C:\Camera_Recordings", type=str, help="Path where to store the captured data")
...
# could use dai.Device.getXLinkChunkSize to get poe speed???????
        ...
        #!!!check encoding settings to configure with 60 fps
        SKIP_FRAMES = roundUp(1.5 * args.fps, 30 if args.quality == "LOW" else 1) 
        args.frame_cnt += SKIP_FRAMES

        # Terminate app handler
        quitEvent = threading.Event()
        #!!! this is the deffinition of the stop recording function, this will need to be changed to be coompatable with the IMU (SIGTERM)
                    ...
                    for q in recording.queues:
                        ...
                        if new_msg is not None:
                            ...
                            if checkSync(recording.queues, new_msg.getSequenceNum()):
                                ...
                                # !!we will need to change the auto focus (this may also be fine given that we dont come too close)
